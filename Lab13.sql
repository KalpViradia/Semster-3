...
SELECT Village.Name FROM City JOIN Village ON City.CityID = Village.CityID WHERE City.CityID = 1;

--2. Display city along with their villages & pin code.
SELECT City.Name, Village.Name, City.Pincode FROM City JOIN Village ON City.CityID = Village.CityID;

--3. Display the city having more than one village.
SELECT City.Name FROM City JOIN Village ON City.CityID = Village.CityID GROUP BY City.Name HAVING COUNT(Village.Name) > 1;

--4. Display the city having no village.
SELECT City.Name FROM City LEFT JOIN Village ON City.CityID = Village.CityID GROUP BY City.Name HAVING COUNT(Village.Name) = 0;

--5. Count the total number of villages in each city.
SELECT City.Name, COUNT(Village.Name) AS VILLAGE_COUNT FROM City JOIN Village ON City.CityID = Village.CityID GROUP BY City.Name;

--6. Count the number of cities having more than one village.
SELECT COUNT(DISTINCT City.Name) AS CITY_COUNT FROM City JOIN Village ON City.CityID = Village.CityID WHERE City.Name IN (SELECT City.Name FROM City JOIN Village ON City.CityID = Village.CityID GROUP BY City.Name HAVING COUNT(Village.Name) > 1);
